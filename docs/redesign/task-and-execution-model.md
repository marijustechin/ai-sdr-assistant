# Redesign — Task and Execution Model
...
Statuses (mirrors the v2 `OperationStatus`, extended):
...
- The **worker process** is a separate process (`workers/*`) that imports the **same** module application services and `packages/database`. It never speaks HTTP to `apps/api`.
...
- Budgets are defined in each run manifest (see the harness in `legacy/docs/market-research-harness.md`) and enforced by the module.
