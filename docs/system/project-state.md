...
follow-up scheduling). **Supplier price inquiry was decommissioned from Market
Research on 2026-09-25** (see `decisions.md`): its endpoints are unregistered,
the research-result module removed, and its generic mailbox/reply code and tables
retained dormant — historical records preserved, no drops. **Also 2026-09-25:
human outreach exclusion decisions + structured sender identity** (see the
"Human outreach decisions" row). Earlier, **O-018
...
**Supplier price inquiry is decommissioned as a Market Research capability
(2026-09-25).** The `price-inquiry` and `quote-collection` modules and their
controllers are unregistered (dormant retained infrastructure), the
`research-result` module is removed, and no Market Research flow creates supplier
inquiry drafts or follow-ups. Public absence of a price remains a recorded
evidence finding ("Price not recorded"), never an outreach trigger. The generic
`email-accounts`, Message-ID tracking, account-wide reply scanning, inbound
matching/parsing and follow-up code/tables are retained for future sales-outreach
reuse. Historical rows are preserved and **no table is dropped**; a future
human-approved migration may drop `research_results`, `price_inquiry_drafts`,
`quote_outbound_messages`, `quote_inbound_messages`, `supplier_quotes` and
`quote_follow_ups` after export/archival. Buyer outreach (leads, contacts,
sender profiles, outreach drafts) is unchanged.
