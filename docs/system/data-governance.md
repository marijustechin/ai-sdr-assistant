...
| `sender_profiles` | Reusable, product-independent **sender identities** (label, sender name, optional **role/title**, optional **company/brand**, From/Reply-To, optional **phone**/**website**, `whatsapp_enabled` (default false) + optional `whatsapp_phone`, status) + optional `email_account_id`; owns **no** transport credentials. WhatsApp is **display metadata** for the closing only (never a sending permission); the dedicated number falls back to the main phone and enabling it without either is rejected. A free-text `signature` is retained but **deprecated**; generated closing is composed from the structured fields only, and a canonical role/title is never translated | `sender-profiles` | impl |
...
| `outreach_drafts` | Evidence-backed **initial** outreach drafts: recipient reference, subject/body, language, preparation status (`PREPARED \| BLOCKED`), rationale, context/evidence references, sender profile reference + non-secret identity snapshot, and precise missing fields; append-only/versioned + idempotent fingerprint; a generated `html_body` (text body + HTML signature, optional logo) is stored alongside the plain-text body; no send state | `outreach-drafter` | impl |
| `outreach_decisions` | Human outreach eligibility decision scoped to one opportunity + company (lead scope): `ELIGIBLE \| DO_NOT_CONTACT \| EXISTING_RELATIONSHIP \| NOT_RELEVANT \| ALREADY_CONTACTED` + optional note; explicit human provenance (`decided_by_kind = HUMAN`, `decided_at`). Any non-`ELIGIBLE` state overrides the agent gate and blocks outreach drafting; never a global blacklist and never hidden from research. Recordable directly from a research offering **before any lead exists** (a lead is never required) | `outreach-drafter` | impl |
